...
namespace Engine
{
	Sprite3DShader::Sprite3DShader(Graphics::Direct3D* d3d, IEngineSettings* settings) : Shader(d3d)
	{
	...
	void Sprite3DShader::createResources()
	{
	...
	void Sprite3DShader::updateResources()
	{
	...
	void Sprite3DShader::renderResources(IShaderResources* resources)
	{
		SpriteShaderResources* res = dynamic_cast<SpriteShaderResources*>(resources);
		renderResources(res);
	}

	int Sprite3DShader::addTexture(ID3D11ShaderResourceView* texture)
	{
	...
	void Sprite3DShader::renderResources(SpriteShaderResources* resources)
	{
	...
	void Sprite3DShader::setSprite(SpriteData* sprite, float left, float top, float right, float bottom, UINT tex_id)
	{
	...
	bool Sprite3DShader::addSprite(Vector4 coords, Vector3 position, ID3D11ShaderResourceView* texture)
	{
		if (m_resources->num_sprites < max_sprites_count)
		{
			...
			if (tex_id >= 0) {
				...
				InstanceData* instance = &instanceData[m_resources->num_sprites];

				instance->tex_id = tex_id;
				instance->position = position;
				
				setSprite(sprite, coords.x, coords.y, coords.z, coords.w, static_cast<UINT>(tex_id) );
				
				m_resources->num_sprites++;
				need_update = true;
